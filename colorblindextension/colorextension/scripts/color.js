...
const applyFilterToImage = (imgIndex, imageList, nameID) => {
	...
	thisImage.onload = () => {
		if (filterName == 0) {
			...
			console.log("WIDTH: ", thisCanvas.width);
			console.log("HEIGHT: ", thisCanvas.height);
...
const savePresetBtn = createButtonWithFunction("Save Preset", savePreset);
const loadPresetBtn = createButtonWithFunction("Load Preset", loadPreset);
const deuteranopiaBtn = createButtonWithFunction("Deuteranopia", loadPreset);
const tritanopiaBtn = createButtonWithFunction("Tritanopia", tritan);
const achromatopsiaBtn = createButtonWithFunction("Achromatopsia", loadPreset);
...
document.addEventListener("mouseover", function (event) {
	if (event.target.getAttribute("id") == "deuter") {
		...
		colorblindPopup.innerText =
			"Deuteranopia has reduced sensitivity to green, so green colors are reduced.  ";
		...
		colorblindPopup.innerText =
			"Tritanopia has reduced sensitivity to blue, so blue colors are reduced. ";
		colorblindPopup.style.visibility = "visible";
		...
		colorblindPopup.innerText =
			"Achomatropia has reduced sensitivity to all colors, so higher contrast is used.";
		colorblindPopup.style.visibility = "visible";
		...
		colorblindPopup.innerText =
			"Protanopia has reduced sensitivity to red, so red colors are reduced. ";
		colorblindPopup.style.visibility = "visible";
